...
for y = 22:35 %eruption
ee = int2str(y);
outdir=strcat('../../Polarization_out_single_select_SNR5/eruption_',ee);
...
fid = fopen(ofile,'w');
phfile = strcat('../../Phase_shift_single/eruption_',ee,'/',sta,'_phase_shift_stage.',stage,'.',freq,'hz.txt');
snrfile = strcat('../../SNR/eruption_',ee,'/',sta,'_SNR_stage.',stage,'.',freq,'hz.txt');%914_SNR_stage.0.1-5hz.txt
...
%if shift<=thur
if (shift>=lower_thres) && (shift<=higher_thres) && (avgsnr(i) >= 5.0)
workdir= strcat('../../ERUPTION_v2/',sta,'/',freq,'hz/eruption_',ee,'/');
