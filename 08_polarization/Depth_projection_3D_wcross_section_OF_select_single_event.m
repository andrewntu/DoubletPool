...
pfile = strcat('../../Polarization_out_single_select/eruption_',ee,'/',sta,'_polarization_all_4project_stage.',stage,'_select.txt')
...
figure(1);
h = scatter3(0,0,0,'b*');hold on;axis([-210 160 -140 160 -100 10]); %plot Old Faithful  
...
outdir= strcat('../../Projection_single_select/');
if not(isfolder(outdir))
  mkdir(outdir)
end

outdir= strcat('../../Projection_single_select/eruption_',ee);
if not(isfolder(outdir))
  mkdir(outdir)
end
...
outdirmat= strcat('../../Projection_mat_single_select/eruption_',ee);
...
outdir=strcat('../../Projection_mat_single_select/');
if not(isfolder(outdir))
	mkdir(outdir)
end
mat2 = '../../Projection_mat_single_select/cross-Z.mat';
save(mat2,'crossall')


% save the station location in mat
sta = [y,x,-z];
mat3 = strcat('../../Projection_mat_single_select/staloc_select.mat');
save(mat3,'sta')
