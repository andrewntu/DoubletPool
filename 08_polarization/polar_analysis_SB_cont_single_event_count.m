...
outdir=strcat('../../Polarization_out_single_count/');
if not(isfolder(outdir))
    mkdir(outdir)
end
outdir=strcat('../../Polarization_out_single_count/eruption_',ee);
if not(isfolder(outdir))
    mkdir(outdir)
end
...
fid = fopen(ofile,'w');
[stap2] = textread(['../../sta_list_914_select.txt'],'%s','headerlines',0);
