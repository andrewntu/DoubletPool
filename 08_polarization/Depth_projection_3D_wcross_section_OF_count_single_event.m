...
pfile = strcat('../../Polarization_out_single_count/eruption_',ee,'/',sta,'_polarization_all_4project_stage.',stage,'_select.txt')
...
outdir= strcat('../../Projection_single_count/');
if not(isfolder(outdir))
  mkdir(outdir)
end

outdir= strcat('../../Projection_single_count/eruption_',ee);
if not(isfolder(outdir))
  mkdir(outdir)
end
...
outdirmat= strcat('../../Projection_mat_single_count/eruption_',ee);
...
outdir=strcat('../../Projection_mat_single_count/');
if not(isfolder(outdir))
	mkdir(outdir)
end
mat2 = '../../Projection_mat_single_count/cross-Z.mat';
save(mat2,'crossall')


% save the station location in mat
sta = [y,x,-z];
mat3 = strcat('../../Projection_mat_single_count/staloc_select.mat');
save(mat3,'sta')
