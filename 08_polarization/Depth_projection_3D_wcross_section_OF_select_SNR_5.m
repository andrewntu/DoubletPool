...
pfile = strcat('../../Polarization_out_phase_SNR_5/',sta,'_polarization_all_4project_stage.',stage,'_select.txt')
...
figure(1);
h = scatter3(0,0,0,'b*');hold on;axis([-210 160 -140 160 -100 40]); %plot Old Faithful  
...
h.MarkerFaceColor = [0.9 0 0];
pic1 = strcat('../../Projection_fig_select_SNR_5/Polarization_stage.',stage,'.png');
pic1s = strcat('../../Projection_fig_select_SNR_5/Polarization_stage.',stage,'.fig');
saveas(figure(1),pic1);
savefig(figure(1),pic1s,'compact');

% save the hit count matrix
mat = strcat('../../Projection_mat_select_SNR_5/polarization_project_total_stage.',stage,'.mat')
save(mat,'total')
...
pic2 = strcat('../../Projection_fig_select_SNR_5/Surf_cross_section_stage.',stage,'.png');
saveas(figure(2),pic2);

% % save the station location in mat
staa = [y,x,-z];
mat3 = strcat('../../Projection_mat_select_SNR_5/staloc_select_',stage,'.mat');
save(mat3,'staa')

end  %time

% % save the cross-section grid file for all the time windows (use this to have a better plot in Pyhton)
% mat2 = '../../Projection_mat_select_SNR_5/cross-Z.mat';
% save(mat2,'crossall')


% % save the station location in mat
sta = [y,x,-z];
mat3 = strcat('../../Projection_mat_select_SNR_5/staloc_select.mat');
save(mat3,'sta')
