...
% for s = 1:1
	...
	h = scatter3(0,0,0,'b*');hold on;axis([-75 75 -75 75 -30 10]);   
	stage = char(stage_all(s))
	smark = strcat('Stage ',stage);
	pfile = strcat('../../Polarization_out_stack/',sta,'_polarization_all_4project_stage.',stage,'_',freq,'hz.txt')
...
% 	gy = -50:2.5:50;
% 	gz = -50:2.5:2.5;  
%     crosgrid_x=length(gx);
%     crosgrid_z=length(gz);
%     total = zeros(length(gx),length(gy),length(gz));
	...
	outdir= strcat('../../Projection_fig_stack/');
	if not(isfolder(outdir))
	mkdir(outdir)
	end

	pic1 = strcat(outdir,'/Polarization_stage.',stage,'_',freq,'hz.png');
	saveas(figure(1),pic1);

	outdirmat= strcat('../../Projection_mat_stack/');
	if not(isfolder(outdirmat))
	mkdir(outdirmat)
	end

	mat = strcat(outdirmat,'/polarization_project_total_stage.',stage,'_',freq,'hz.mat')
	save(mat,'total')
	...
	figure(2);
	crossall(s,:,:)=sur;
	surf(xx,zz,sur');caxis([1 15]);view(0,90);colormap(flipud(hot));hold on;
	%colorbar;title(smark);axis([-100 50 min(gz) -3]);grid off;
    colorbar;title(smark);axis([-100 50 -50 5]);grid off;
	% save the cross-section plot
	pic2 = strcat(outdir,'/Surf_cross_section_stage.',stage,'_',freq,'hz.png');
...
sta = [y,x,-z];
mat3 = strcat('../../Projection_mat_stack/staloc_select.mat');
save(mat3,'sta')


mat2 = '../../Projection_mat_stack/cross-Z.mat';
save(mat2,'crossall')
